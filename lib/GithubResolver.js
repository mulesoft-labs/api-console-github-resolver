...
import { GithubCache } from './GithubCache.js';
...
export class GithubResolver {
  ...
  async _makeRequest(url, headers) {
    ...
    const etag = await this._cache.lastEtag(url);
    if (etag) {
      headers['if-None-Match'] = etag;
    }
    const result = await this._transport.get(url, headers);
    if (this._transport.latestStatus === 304) {
      return await this._cache.getCachedResult(url);
    }
    this._handleResponseHeaders();
    try {
      const etag = this._transport.latestHeaders.etag;
      if (etag) {
        await this._cache.storeResponse(url, etag, result);
      }
    } catch (e) {
      this.logger.warn(e);
    }
  ...
  async getLatestInfo() {
    ...
    const info = await this.getReleasesList();
    return info[0];
  }
  /**
  ...
  async getReleasesList() {
    ...
    let info = await this._makeRequest(this._githubReleasesUrl, this._infoHeaders);
  ...
  async getTagInfo(tag) {
    ...
    try {
      const result = await this._makeRequest(url, this._infoHeaders);
      if (this._transport.latestStatus === 404) {
        await this._throwTag404Error(tag);
      }
      return result;
    } catch (e) {
      if (this._transport.latestStatus === 404) {
  ...
  /**
   * Builds error message when requested tag does not exists.
   * @param {String} tag The tag tyhat does not exists.
   * @return {Promise}
   * @throws {Error}
   */
  async _throwTag404Error(tag) {
    this.logger.debug('[GithubResolver] Tag does not exists.');
    const list = await this.getReleasesList();
    const message = this._getReleasesListErrorMessage(tag, list);
    throw new Error(message);
  }
   ...
   * @param {Array<Object>} releases List of releases to the repository.
