...
class GithubResolver {
   ...
   *
   * @return {Promise} Returns a JavaScript object with the response from GitHub.
   */
  getLatestInfo() {
  ...
  _getReleasesListErrorMessage(tag, releases) {
    let message = 'Tag for release ' + tag + ' do not exists in API Console repository. ';
    message += 'Please, check if you are requesting a valid tag. ';
    message += 'Available tags are: ';
    //jscs:disable
    let tags = releases.map((item) => item.tag_name);
    //jscs:enable
  ...
  _assertTag(tag) {
    ...
    if (major < minVersion) {
      throw new Error(
        `This tools will not work with API Console release prior version ${minVersion}.0.0`
      );
    }
  }
}
